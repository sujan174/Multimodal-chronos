"""
Hybrid Multi-Modal Stock Prediction System with LoRA Fine-Tuning
Fuses EfficientNet (visual/wavelet) + LoRA-Chronos (time-series) + Technical Indicators
with Cross-Attention and Multi-Head Fusion

LIGHTWEIGHT VERSION - Optimized for 16GB RAM
"""
...
try:
    ...
    print("[INFO] Chronos and PEFT available - using LoRA fine-tuning")
except ImportError as e:
    CHRONOS_AVAILABLE = False
    print(f"[WARNING] Chronos/PEFT not available: {e}")
    print("[INFO] Install with: pip install chronos-forecasting peft")
    raise RuntimeError("Chronos and PEFT are required for this model")
...
class EfficientNetFeatureExtractor(nn.Module):
    def __init__(self, pretrained_path):
        ...
        print("[EfficientNet] Loaded pretrained weights and froze parameters")
...
class LoRAChronosEncoder(nn.Module):
    def __init__(self, model_name="amazon/chronos-t5-tiny", device="cpu", lora_rank=8, lora_alpha=16):
        ...
        print(f"[LoRA-Chronos] Loading {model_name}...")
        ...
        print(f"[LoRA-Chronos] Trainable params: {trainable_params:,} ({100*trainable_params/total_params:.2f}%)")
        ...
        print(f"[LoRA-Chronos] Model ready on {target_device}")
            ...
            with torch.enable_grad():
                try:
                    ...
                    if hasattr(base_model, 'encoder'):
                        ...
                        raise AttributeError(f"Cannot find encoder in model structure")
                    ...
                    all_hidden.append(torch.zeros(1, 256, device=self.device))
...
class ChronosFeatureExtractor:
    def __init__(self, model_name=CHRONOS_MODEL_NAME, device=DEVICE, use_lora=True):
        ...
        print(f"[TimeSeries] Initializing LoRA-adapted Chronos...")
        ...
        print(f"[TimeSeries] Encoder ready on {device}")
...
class HybridFusionModel(nn.Module):
    def __init__(self, efficientnet_path, chronos_model_name, num_technical_features):
        ...
        print(f"[MODEL] Time-series embedding dim: {timeseries_dim}")
        print(f"[MODEL] Visual embedding dim: {visual_dim}")
        print(f"[MODEL] Technical embedding dim: {TECHNICAL_FEATURE_DIM}")
...
class HybridMultiModalDataset(Dataset):
    def __init__(self, df, scalogram_path, transform, scaler=None, fit_scaler=False):
        ...
        print(f"[Dataset] {len(self.technical_cols)} technical features extracted")
    ...
    def __getitem__(self, idx):
        ...
        time_series_data = self.df.iloc[max(0, idx - CHRONOS_CONTEXT_LENGTH):idx]['close'].values
        time_series = torch.tensor(time_series_data, dtype=torch.float32)
...
def fetch_and_prepare_data(tickers, start_date, end_date, scalogram_path):
    ...
    for ticker in tqdm(tickers, desc="Processing Tickers"):
        ...
        try:
            ...
            print(f"[ERROR] Failed to calculate indicators for {ticker}: {e}")
            failed_tickers.append(ticker)
            continue
        ...
        if dropped > 0:
            print(f"[INFO] {ticker}: Dropped {dropped} rows with NaN values")
        
        df = df.loc[df.index >= start_date].copy()
        
        if len(df) < 10:
            print(f"[SKIP] {ticker}: Too few valid rows after filtering ({len(df)})")
            failed_tickers.append(ticker)
            continue
        
        print(f"[DEBUG] {ticker}: Data range {df.index.min()} to {df.index.max()}, {len(df)} rows")
        
        try:
            generate_wavelet_scalograms(df, scalogram_path, ticker)
        except Exception as e:
            print(f"[ERROR] Failed to generate scalograms for {ticker}: {e}")
            failed_tickers.append(ticker)
            continue
        ...
        if missing_images > 0:
            print(f"[WARNING] {ticker}: {missing_images} images not found (out of {initial_image_count})")

        df = df[exists_mask].copy()
        
        if len(df) > 0:
            print(f"[SUCCESS] {ticker}: {len(df)} valid samples with images")
            all_data.append(df)
        else:
            print(f"[SKIP] {ticker}: No valid images found")
            failed_tickers.append(ticker)
    ...
    if len(all_data) == 0:
        print(f"\n[ERROR] No valid data collected from any ticker!")
        return None
    
    combined = pd.concat(all_data, ignore_index=False)
    print(f"\n[SUCCESS] Total samples collected: {len(combined)}")
    return combined
...
def generate_wavelet_scalograms(df, output_path, ticker):
    ...
    if generated_count > 0:
        print(f"[SCALOGRAM] {ticker}: Generated {generated_count} new images")
    if skipped_count > 0:
        print(f"[SCALOGRAM] {ticker}: Skipped {skipped_count} existing images")
...
def train_hybrid_model(train_loader, val_loader, model, device):
    ...
    print("[OPTIMIZER] Including LoRA parameters from Chronos encoder")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print(f"[WEIGHTS] Visual: {weights['visual']:.3f} | "
              f"TimeSeries: {weights['timeseries']:.3f} | "
              f"Technical: {weights['technical']:.3f}")
...
def main():
    print("="*80)
    print("HYBRID MULTI-MODAL STOCK PREDICTION SYSTEM")
    print("EfficientNet + LoRA-Chronos + Technical Indicators Fusion")
    print("LIGHTWEIGHT VERSION - Optimized for 16GB RAM")
    print("="*80)
    ...
    print(f"\n[CONFIG] Training with {len(TRAIN_TICKERS)} stocks")
    print(f"[CONFIG] Period: {TRAIN_START} to {TEST_END}")

    print("\n[DATA] Preparing training data...")
    train_df = fetch_and_prepare_data(TRAIN_TICKERS, TRAIN_START, VAL_START, SCALOGRAM_PATH)
    
    if train_df is None:
        raise RuntimeError("[ERROR] Failed to prepare training data")
    
    print(f"\n[TRAIN DATA] Successfully prepared {len(train_df)} training samples")
    
    print("\n[DATA] Preparing validation data...")
    val_df = fetch_and_prepare_data(TRAIN_TICKERS, VAL_START, TEST_START, SCALOGRAM_PATH)
    
    if val_df is None:
        raise RuntimeError("[ERROR] Failed to prepare validation data")
    
    print(f"\n[VAL DATA] Successfully prepared {len(val_df)} validation samples")
    
    print("\n[DATA] Preparing test data...")
    test_df = fetch_and_prepare_data(TRAIN_TICKERS, TEST_START, TEST_END, SCALOGRAM_PATH)
    
    if test_df is None:
        raise RuntimeError("[ERROR] Failed to prepare test data")
    
    print(f"\n[TEST DATA] Successfully prepared {len(test_df)} test samples")
    ...
    print("\n[MODEL] Initializing Hybrid Fusion Model...")
    
    num_technical_features = len(train_dataset.technical_cols)
    print(f"[MODEL] Technical features: {num_technical_features}")
    ...
    print(f"\n[FINAL WEIGHTS]")
    print(f"  Visual (EfficientNet): {final_weights['visual']:.3f}")
    print(f"  TimeSeries (LoRA-Chronos): {final_weights['timeseries']:.3f}")
    print(f"  Technical Indicators: {final_weights['technical']:.3f}")
    
    results = {
        'config': {
            'efficientnet_model': EFFICIENTNET_MODEL_PATH,
            'timeseries_encoder': "LoRA-adapted Chronos-T5-Tiny",
    ...
    print(f"✓ Results saved: {RESULTS_PATH}/final_results.json")
