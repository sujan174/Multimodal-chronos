...
# ============================================================================
# DATE CONFIGURATION - CHANGE THESE TO MODIFY TRAINING PERIOD
# ============================================================================
TRAIN_START = "2022-06-01"   # Training data start
VAL_START = "2024-01-01"     # Validation data start (Training ends here)
TEST_START = "2024-07-01"    # Test data start (Validation ends here)
TEST_END = "2024-12-31"      # Test data end
...
class EfficientNetFeatureExtractor(nn.Module):
    """Extracts frozen features from pretrained EfficientNet"""
...
class LSTMTimeSeriesEncoder(nn.Module):
    """Fallback encoder when Chronos unavailable"""
...
class LoRAChronosEncoder(nn.Module):
    """Time series encoder using LoRA-adapted Chronos"""
...
class ChronosFeatureExtractor:
    """Wrapper for time series encoding with LoRA-Chronos or LSTM fallback"""
...
class CrossAttentionFusion(nn.Module):
    """Fuses visual, time series, and technical features with cross-attention"""
...
class HybridFusionModel(nn.Module):
    """Main model combining EfficientNet, Chronos/LSTM, and technical indicators"""
...
class HybridMultiModalDataset(Dataset):
    """Dataset combining scalogram images, time series, and technical indicators"""
...
def get_transforms(augment=False):
    """Image transforms with optional augmentation"""
    if augment:
...
def fetch_and_prepare_data(tickers, start_date, end_date, scalogram_path):
    """Download stock data, calculate indicators, generate scalograms"""
    all_data = []
    failed_tickers = []
...
def generate_wavelet_scalograms(df, output_path, ticker):
    """Generate RGB scalogram images from price data"""
...
def evaluate(model, loader, criterion, device, split_name="VAL", analyze_confidence=True):
    ...
    if analyze_confidence and len(confidences) > 0:
        ...
        if high_conf_mask.sum() > 0:
            ...
            print(f"  High (≥{high_conf_threshold:.3f}): Acc={high_conf_acc:.2f}%, F1={high_conf_f1:.3f}, N={high_conf_mask.sum()}")
        ...
        if med_conf_mask.sum() > 0:
            ...
            print(f"  Medium: Acc={med_conf_acc:.2f}%, F1={med_conf_f1:.3f}, N={med_conf_mask.sum()}")
        ...
        if low_conf_mask.sum() > 0:
            ...
            print(f"  Low: Acc={low_conf_acc:.2f}%, F1={low_conf_f1:.3f}, N={low_conf_mask.sum()}")
