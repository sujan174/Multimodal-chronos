# efficientnet_forecaster_v2.py
"""
Production-grade multi-stock price prediction system using EfficientNet fine-tuning
with best ML practices including progressive unfreezing, discriminative learning rates,
early stopping, mixed precision, and comprehensive monitoring.
"""
...
GRAD_ACCUM_STEPS = 2
LEARNING_RATE = 1e-3
BACKBONE_LR_RATIO = 0.1  # Backbone gets 1/10 of main LR
WEIGHT_DECAY = 1e-4
DROPOUT = 0.4
LABEL_SMOOTHING = 0.1  # Prevent overconfidence

# Training Strategy
PHASE1_EPOCHS = 5  # Warmup: frozen backbone
PHASE2_EPOCHS = 15  # Fine-tune: unfrozen backbone
...
def generate_wavelet_scalograms(prices_df, output_path, ticker):
    """Generate scalogram images for wavelet analysis"""
    ...
    for i in range(LOOKBACK_WINDOW, len(price_series)):
        ...
        # Generate 3-channel scalogram (RGB)
        channels = []
...
# =============================================================================
# MODEL ARCHITECTURE WITH PROPER FINE-TUNING
# =============================================================================
class EfficientNetPredictor(nn.Module):
    def __init__(self, dropout=DROPOUT, freeze_backbone=True):
        ...
        if freeze_backbone:
            ...
            print("[MODEL] Backbone frozen for Phase 1")
    ...
    def unfreeze_backbone(self, progressive=True, num_layers=None):
        ...
        Args:
            progressive: If True, unfreeze only last few layers
        ...
        if progressive and num_layers:
            ...
            print(f"[MODEL] Unfrozen last {num_layers} backbone layers")
        else:
            for param in self.backbone.features.parameters():
                param.requires_grad = True
...
class MetricsTracker:
    """Track and log metrics across training"""
    def __init__(self):
        self.history = {
            'train_loss': [],
            'val_loss': [],
...
# =============================================================================
# MAIN TRAINING LOOP WITH BEST PRACTICES
# =============================================================================
def train_model(train_loader, val_loader, test_loader, device, class_weights=None):
    """
    Two-phase fine-tuning with best ML practices:
    1. Phase 1: Train classifier head with frozen backbone
    2. Phase 2: Fine-tune entire model with discriminative learning rates
    """
    print("\n" + "="*80)
    ...
    if class_weights is not None:
        ...
        print(f"[LOSS] Using class weights: {class_weights} | Pos weight: {pos_weight_value:.3f}")
    else:
        criterion = nn.BCEWithLogitsLoss()
    ...
    # =========================================================================
    print("\n" + "="*80)
    print("PHASE 1: WARMUP - Training Classifier Head (Frozen Backbone)")
    print("="*80)
    ...
    for epoch in range(PHASE1_EPOCHS):
        ...
        if val_metrics['f1_score'] > best_val_f1:
            ...
            print(f"[SAVE] Phase 1 checkpoint saved (F1: {best_val_f1:.3f})")
    ...
    # =========================================================================
    print("\n" + "="*80)
    print("PHASE 2: FINE-TUNING - Full Model Training (Unfrozen Backbone)")
    print("="*80)
    ...
    for epoch in range(PHASE2_EPOCHS):
        ...
        if val_metrics['f1_score'] > best_val_f1:
            ...
            print(f"[SAVE] Best model saved (F1: {best_val_f1:.3f})")
    ...
    print(f"[LOAD] Best model from epoch {checkpoint['epoch']} (F1: {checkpoint['val_f1']:.3f})")
...
def main():
    ...
    # =========================================================================
    print("\n[DATA] Fetching and processing stock data...")
    ...
    print(f"\n[SUCCESS] Processed {len(successful_tickers)}/{len(TICKERS)} tickers")
...
def plot_training_history(history_path='./models/training_history.json'):
    ...
    try:
        ...
        print(f"[PLOT] Training history saved to ./models/training_history.png")
        
    except ImportError:
        print("[WARNING] matplotlib not installed. Skipping plot generation.")
    except Exception as e:
        print(f"[ERROR] Failed to plot training history: {e}")


def analyze_per_ticker_performance(model, test_loader, test_combined, device):
    """
    Analyze model performance per ticker
    """
    model.eval()
    ticker_predictions = {}
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"[INFO] Skipping plot generation: {e}")
